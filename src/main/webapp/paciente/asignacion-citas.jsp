...
<html>
    <head>
        ...
        <title>Agendar - Sistema de Citas Medicas</title>
    ...
    <body>
        ...
        <h3 class="text-center my-4" style="color: #439CD5">ASIGANCION DE CITAS</h3>
        ...
        <div class="container">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-10" style="background-color:  #fff; padding: 1rem">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <h5 class="mb-2">SELECCIONE EL SERVICO</h5>
                            <form action="/ResultadoBusquedaSv" method="GET">
                                <select class="form-select mb-2" name="txtEspecialidad" required>
                                    <option></option>
                                    <%
                                        LogicaEspecialidad logicaEspecialidad = new LogicaEspecialidad();
                                        List<Especialidad> listaEspecialdad = logicaEspecialidad.obtenerEspecialidades();
                                        if (listaEspecialdad != null) {
                                            for (Especialidad result : listaEspecialdad) {

                                    %>
                                    <option value="<%=result.getId()%>"><%=result.getNombre()%></option>
                                    <% }
                                        }%>
                                </select>
                        ...
                        <div class="card-body">             
                            <table id="datatablesSimple" class="table">
                                ...
                                <tbody>
                                    ...
                                    <%
                                        List<MedicoDTO> lista = (List) request.getSession().getAttribute("listaResultado");
                                        SimpleDateFormat formatoFecha = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                        if (lista != null) {
                                            for (MedicoDTO result : lista) {
                                    ...
                                    <tr>
                                        <td><%=result.getNombres() + " " + result.getApellidos()%></td>
                                        <td><%=result.getEspecialidad()%></td>
                                        <td><%=result.getSede()%></td>
                                        <td><%=fechaFormateada%></td>
                                        <td >
                                            <form name="agendar" action="/ResultadoBusquedaSv" method="POST" class="text-center">
                                                <button type="submit" class="btn btn-success">
                                                    <i class="fas fa-calendar-plus"></i>
                                                </button>
                                                <input type="hidden" name="fecha" value="<%=fechaFormateada%>">
                                                <input type="hidden" name="idMedico" value="<%=result.getId()%>">
                                                <input type="hidden" name="idPaciente" value="<%=usuario.getNumeroDocumento()%>">
